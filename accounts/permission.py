    ...
    def decorator(view_func):
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not iUser.is_superuser:
                ...
                # 需排除的权限
                for x in role_permission_list:
                    if str(x.url).startswith('!'):
                        t_x_url = x.url[1:].rstrip('/') + '/'
                        if str(request.path + '/').startswith(t_x_url):
                            # matchUrl= []
                            # break
                            return HttpResponseRedirect(reverse('permission_deny'))
                        matchUrl = [url for url in matchUrl if not str(url).startswith(t_x_url)]
