...
def job_backend(request):
    # celery_file = os.path.exists('/var/opt/alerts/pid/w1.pid')
    celery_file = os.path.exists('/var/opt/alerts/pid/celery.pid')
    beat_file = os.path.exists('/var/opt/alerts/pid/beat.pid')
    if celery_file:
...
def job_backend_task(request, name, action):
    ...
    cmd = ' '.join(["supervisorctl", action, name])
    Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    if action == 'stop':
        cmd = "rm -f /var/opt/alerts/pid/{0}.pid".format(name)
        Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
    time.sleep(3)
